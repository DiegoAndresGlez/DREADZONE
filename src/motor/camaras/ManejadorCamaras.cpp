...
ManejadorCamaras::ManejadorCamaras()
{
...
void ManejadorCamaras::set_camara(Camara &cam)
{
  get().camara_actual = &cam;
};
...
void ManejadorCamaras::renderizar_ang(std::vector<Objeto*> objetos, Coordenadas target)
{
  get().camara_actual->renderizar_ang(objetos, target);
}