...
void Plotter::pintar_textura_ang(SDL_Renderer *r,SDL_Texture *textura,SDL_Rect &src , SDL_Rect &dest, Coordenadas size, Coordenadas target)
{
    ...
    Coordenadas pos = {target.x, target.y};
    Coordenadas centro = {dest.x+dest.w/2,dest.y+dest.h/2}; //centro de la textura
    float angulo = atan2(pos.y-centro.y,pos.x-centro.x)*180/PI; 

    SDL_RenderCopyEx(r,textura,&src,&dest,angulo,NULL,SDL_FLIP_NONE);
    //RenderCopyEx para voltear o hacer flip un sprite
};