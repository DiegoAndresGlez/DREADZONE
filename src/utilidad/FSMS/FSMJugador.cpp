...
FSMJugador* EstadoJugadorIDLE::input_handle(KeyOyente& input, MouseOyente& mouse)
{
    ...
    if(input.estaPresionado(SDL_SCANCODE_W))
        return new EstadoJugadorMOVERDER({0,-1});
    ...
    if(input.estaPresionado(SDL_SCANCODE_W))
        return new EstadoJugadorMOVERARR({0,-1});
    */
    /*
    if(input.estaPresionado(SDL_SCANCODE_S))
...
FSMJugador* EstadoJugadorMOVERDER::input_handle(KeyOyente& input, MouseOyente& mouse)
{
    ...
    if(input.estaPresionado(SDL_SCANCODE_W) & input.estaPresionado(SDL_SCANCODE_D))
        return new EstadoJugadorMOVERARR({1,-1});
    if(input.estaPresionado(SDL_SCANCODE_W) & input.estaPresionado(SDL_SCANCODE_A))
        return new EstadoJugadorMOVERARR({-1,-1});
    */
    /*
...
/*MOVER ARRIBA*/
EstadoJugadorIDLEARR::EstadoJugadorIDLEARR()
{
    strnombre = "IDLEARRIBA";
    frames_actual_ani = 0;
    frames_maxim_ani = 1;
}

void EstadoJugadorIDLEARR::entrar(Jugador& player)
{
    frames_actual_ani = 0;
    frames_maxim_ani = 1;
}

void EstadoJugadorIDLEARR::salir(Jugador& player){}

void EstadoJugadorIDLEARR::update(Jugador& player, double dt)
{
...
FSMJugador* EstadoJugadorIDLEARR::input_handle(KeyOyente& input, MouseOyente& mouse)
{
    if(input.estaPresionado(SDL_SCANCODE_D))
        return new EstadoJugadorMOVERDER({1,0});
    if(input.estaPresionado(SDL_SCANCODE_A))
...
EstadoJugadorMOVERARR::EstadoJugadorMOVERARR(Coordenadas dir)
{
    strnombre = "MOVERARRIBA";
    frames_actual_ani = 0;
    frames_maxim_ani = 8;
    velocidad = 5;
    direccion = dir;
...
void EstadoJugadorMOVERARR::entrar(Jugador& player)
{
    frames_actual_ani = 0;
    frames_maxim_ani = 8;
}

void EstadoJugadorMOVERARR::salir(Jugador& player){}

void EstadoJugadorMOVERARR::update(Jugador& player, double dt)
{
    Coordenadas p = player.get_posicion_mundo();
    if (player.en_colision)
    {
...
FSMJugador* EstadoJugadorMOVERARR::input_handle(KeyOyente& input, MouseOyente& mouse)
{
    if(input.estaPresionado(SDL_SCANCODE_S) & input.estaPresionado(SDL_SCANCODE_D))
        return new EstadoJugadorMOVERDER({1,1});
    if(input.estaPresionado(SDL_SCANCODE_S) & input.estaPresionado(SDL_SCANCODE_A))
    ...
    */
    ...
    if(input.nadaPresionado())
        return new EstadoJugadorIDLEARR();
